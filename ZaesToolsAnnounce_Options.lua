...
content:SetSize(1, 1)               -- will expand as we add children
...
local function Note(parent, anchor, text, dy)
  ...
  fs:SetWidth(560); fs:SetJustifyH("LEFT"); fs:SetText(text); return fs
end
local function GroupBox(parent, label, x, y, w, h)
  local box = CreateFrame("Frame", nil, parent, "BackdropTemplate")
  box:SetPoint("TOPLEFT", x, y); box:SetSize(w, h)
  if box.SetBackdrop then
    box:SetBackdrop({ bgFile="Interface\\DialogFrame\\UI-DialogBox-Background-Dark", edgeFile="Interface\\Tooltips\\UI-Tooltip-Border", edgeSize=12, insets={left=6,right=6,top=6,bottom=6} })
  end
  local t = box:CreateFontString(nil, "ARTWORK", "GameFontNormal")
  t:SetPoint("TOPLEFT", 10, -8); t:SetText(label)
  return box, t
end
...
local function NewCheckbox(parent, label, tooltip, onClick)
  ...
  text:SetPoint("LEFT", cb, "RIGHT", 4, 0); text:SetText(label)
...
local function NewDropdown(parent, name, titleText, width, items, getKey, setKey)
  ...
  title:SetPoint("BOTTOMLEFT", dd, "TOPLEFT", 18, 3); title:SetText(titleText)
  ...
  UIDropDownMenu_Initialize(dd, function(self, level)
    ...
    for _, entry in ipairs(items) do
      ...
      info.func = function() setKey(entry.key); UIDropDownMenu_SetSelectedID(dd, getIndexByKey(items, entry.key)) end
...
local CHANNELS = {
  ...
  { key="RAID",  label="Raid" }, { key="PARTY", label="Party" },
  { key="SAY",   label="Say"  }, { key="YELL",  label="Yell" },
  { key="EMOTE", label="Emote"},
}
local FALLBACKS = { { key="SAY", label="Fallback when solo: Say" }, { key="EMOTE", label="Fallback when solo: Emote" }, }
local MARKER_MODES = {
...
local desc  = Note(content, title, "Announce to your group when you interrupt, go OOM (only on failed heal), apply CC, when CC breaks (by who/what), or when it fades. Use /zta or /ztaopt.", -8)

-- Group: Output
local boxOut = GroupBox(content, "Output", 16, -74, 560, 120)
local ddChan = NewDropdown(boxOut, "ZaesToolsAnnounceChannelDropdown", "Announcement Channel", 240, CHANNELS,
...
local ddFallback = NewDropdown(boxOut, "ZaesToolsAnnounceFallbackDropdown", "SMART Solo Fallback", 220, FALLBACKS,
...
-- Group: Throttles
local boxThr = GroupBox(content, "Throttles (seconds)", 16, -324, 560, 170)
...
-- Group: Features
local boxFeat = GroupBox(content, "Features", 16, -504, 560, 130)
...
-- Group: Marker
local boxMark = GroupBox(content, "Marker Display", 16, -644, 560, 80)
local ddMarker = NewDropdown(boxMark, "ZaesToolsAnnounceMarkerModeDropdown", "Marker Display Mode", 260, MARKER_MODES,
...
local boxFmt = GroupBox(content, "Message Formats (press Enter to save)", 16, -734, 560, 220)
local y = -50
...
-- Group: CC List
local boxCC = GroupBox(content, "Crowd Control List (toggle individually)", 16, -964, 560, 260)
local ROWS = 10
local ccRows = {}
...
scrollCC:SetPoint("TOPLEFT", boxCC, "TOPLEFT", 10, -28)
...
for i=1,ROWS do
  local row = CreateFrame("Frame", nil, boxCC); row:SetSize(520, 20); row:SetPoint("TOPLEFT", boxCC, "TOPLEFT", 18, -28-(i-1)*20)
  local check = CreateFrame("CheckButton", nil, row, "InterfaceOptionsCheckButtonTemplate"); check:SetPoint("LEFT", 0, 0)
  local txt = row:CreateFontString(nil, "ARTWORK", "GameFontHighlight"); txt:SetPoint("LEFT", check, "RIGHT", 4, 0); txt:SetWidth(470); txt:SetJustifyH("LEFT")
...
local help = boxCC:CreateFontString(nil, "ARTWORK", "GameFontDisable")
help:SetPoint("TOPLEFT", boxCC, "TOPRIGHT", 8, -4); help:SetWidth(0.1); help:SetText("") -- no overflow text

-- Group: Presets
local boxPres = GroupBox(content, "Template Presets", 16, -1234, 560, 80)
...
dd:SetPoint("TOPLEFT", boxPres, "TOPLEFT", -4, -32)
...
-- Finalize: initialize values on show
panel:SetScript("OnShow", function(self)
  ...
  -- ensure scroll child tall enough
  content:SetHeight(1340)
